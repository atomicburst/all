...
export class ComboFieldRef extends Component<ComboFieldProps & WithRecordField  &WithFocusManagerField & WithLoadView & WithStyles<typeof ComboFieldStyles>, any> {
    id: string;
    items = [];
    ...
    readRemote(displayFieldValue) {
        ...
        return me.loadView.read({limit: 25, offset: 0, viewData: {displayFieldValue: displayFieldValue}}).catch(function (e) {
            setTimeout(function () {
                me.setState({
                    loading:false
                })
            ...
            <div style={{
                ...
                <TextFieldM
                    ...
                    onKeyDown={function (e) {
                        ...
                        let indexActiveRecord = me.loadView.items.indexOf(me.state.activeRecord );
                        if ( indexActiveRecord>-1&& me.state.activeRecord && me.state.open && $event.keyCode == keycode.codes.enter) {
                            ...
                            if (me.loadView.items.length==0){
                                me.search(inputText || "")
                            }
                            $event.preventDefault();
                            let activeIndex = me.loadView.items.indexOf(me.state.activeRecord);
                            let activeRecord;
                            if (activeIndex == -1 || (activeIndex + 1) == me.loadView.items.length) {
                                activeRecord = me.loadView.items[0]
                            } else {
                                activeRecord = me.loadView.items[activeIndex + 1]
                            }
                            me.setState({
                                open: true,
                            ...
                            if (activeIndex == -1) {
                                ...
                                activeRecord = me.loadView.items[me.loadView.items.length - 1]
                            } else {
                                activeRecord = me.loadView.items[activeIndex - 1]
                            }
                            me.setState({
                    ...
                    <Paper className={classes.paper} hidden={!open}>
                        ...
                        >
                            <Async
                                ...
                                then={() => {
                                    return (me.loadView.items).map((record, index) =>
                                        <MenuItem
                                            key={record.id}
                                            id={me.id + '-' + record.id}
